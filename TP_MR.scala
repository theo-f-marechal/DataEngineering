...
object TP_MR {
  ...
  def salarySum(employees: List[Employee]): Double = employees.foldLeft(0.0)((acc, el) => acc + el.salary)
  ...
  def addressOf(users: List[User]): List[String] = users.foldLeft(List.empty[String])((acc, el) => el.address::acc).reverse
